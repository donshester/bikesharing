...
export class DriveService {
  ...
  async startDrive(startDriveDto: StartDriveDto): Promise<DriveEntity> {
    const bike = await this.bikeRepository.findOneBy({id:startDriveDto.bikeId});
    if (!bike) {
      throw new NotFoundException('Bike not found');
    }

    const user = await this.userRepository.findOneBy({id:startDriveDto.userId});
    if (!user) {
  ...
  async endDrive(driveId: number, endDriveDto: EndDriveDto): Promise<DriveEntity> {
